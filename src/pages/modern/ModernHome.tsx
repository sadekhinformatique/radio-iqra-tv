...
export default function ModernHome() {
  ...
  return (
    <main>
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center px-4 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-gradient-to-b from-modern-bg-dark/60 via-modern-bg-dark/80 to-modern-bg-dark z-10"></div>
            ...
            <div className="relative z-20 text-center max-w-4xl mx-auto mt-12">
            ...
            <h2 className="text-5xl md:text-7xl font-black text-white mb-6 leading-tight">{config?.site_name || "Radio Iqra FM"}</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/listen-live" className="bg-modern-gold hover:bg-modern-gold/90 text-modern-bg-dark px-10 py-4 rounded-xl font-black text-lg flex items-center justify-center gap-3 transition-all hover:scale-105 active:scale-95">
                  <span className="material-symbols-outlined text-2xl fill-1">play_arrow</span>
                  Écouter en Direct
                </Link>
                <Link to="/a-propos" className="bg-white/10 hover:bg-white/20 backdrop-blur-md text-white px-10 py-4 rounded-xl font-bold text-lg border border-white/20 transition-all hover:scale-105 active:scale-95">
                  À Propos
                </Link>
            </div>
        </div>
      ...
      {/* Pages Navigation */}
      <section className="py-16 px-4 bg-modern-bg-light/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <Link key={i} to={item.path} className="group bg-modern-neutral-dark rounded-2xl overflow-hidden gold-gradient-border shadow-xl transform transition-all hover:-translate-y-2 p-6 flex flex-col items-center text-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-modern-primary/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <span className="material-symbols-outlined text-3xl text-modern-gold">{item.icon}</span>
                  </div>
                  <h4 className="text-lg font-bold text-white group-hover:text-modern-gold transition-colors">{item.title}</h4>
      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          {/* Latest Podcast */}
          {latestPodcast && (
            <div className="bg-modern-primary/5 rounded-3xl p-8 border border-modern-primary/10 relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="material-symbols-outlined text-9xl">podcasts</span>
              </div>
              <div className="relative z-10">
                <h3 className="text-modern-gold font-bold tracking-widest uppercase mb-6 flex items-center gap-2">
                ...
                <h4 className="text-2xl font-black text-white mb-6 line-clamp-2">{latestPodcast.title || latestPodcast.titre}</h4>
                <button 
                  onClick={() => window.open(latestPodcast.audio_url, '_blank')}
                  className="bg-modern-gold hover:bg-modern-gold/90 text-modern-bg-dark px-6 py-3 rounded-full font-bold text-sm flex items-center gap-2 w-max transition-all hover:scale-105 active:scale-95"
                >
          ...
          {latestArticle && (
            <div className="bg-modern-primary/5 rounded-3xl p-8 border border-modern-primary/10 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-2xl font-black text-white mb-4 line-clamp-2">{latestArticle.title || latestArticle.titre}</h4>
                <p className="text-slate-400 text-sm line-clamp-2 mb-6">{latestArticle.description || latestArticle.content}</p>
                <Link 
                  to="/conseils"
      ...
      <section className="py-16 px-4 mb-16">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl font-black text-white mb-8">À Propos de <span className="text-modern-gold">{config?.site_name || "Iqra FM"}</span></h2>
            <p className="text-lg text-slate-300 leading-relaxed font-serif italic mb-8">
