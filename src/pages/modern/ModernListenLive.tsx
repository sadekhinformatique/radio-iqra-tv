  ...
  return (
    <main className="max-w-7xl mx-auto w-full px-6 lg:px-20 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-8 flex flex-col gap-8">
        {/* Main Player Card */}
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-modern-primary/10 to-transparent border border-modern-primary/20 p-8 lg:p-12">
            ...
            <div className="flex flex-col items-center text-center">
                ...
                <div className="relative size-64 md:size-80 rounded-xl overflow-hidden shadow-2xl border-4 border-modern-gold/20">
                ...
                <h2 className="text-3xl font-black text-slate-900 dark:text-white leading-tight">{config?.site_name || "Iqra FM"}</h2>
                <p className="text-lg text-modern-primary dark:text-modern-gold font-medium">La Voix de la Sagesse</p>
                </div>
                
                {/* Simple Frequency Visualizer */}
                <div className="flex items-end justify-center gap-1.5 h-12 mt-8">
                ...
                <div className="flex items-center justify-center gap-8 mt-10">
                    <button onClick={() => setIsMuted(!isMuted)} className="text-slate-400 hover:text-modern-primary transition-colors">
                        <span className="material-symbols-outlined text-3xl">{isMuted ? 'volume_off' : 'volume_mute'}</span>
                    </button>
                    ...
                    <button 
                      onClick={togglePlay}
                      disabled={isLoading}
                      className={`size-20 bg-modern-primary rounded-full flex items-center justify-center text-white shadow-xl shadow-modern-primary/40 hover:scale-105 active:scale-95 transition-all ${isLoading ? 'opacity-50 cursor-wait' : ''}`}
                    >
                        {isLoading ? (
                    ...
                    <button onClick={() => setIsMuted(!isMuted)} className="text-slate-400 hover:text-modern-primary transition-colors">
                        <span className="material-symbols-outlined text-3xl">{isMuted ? 'volume_off' : 'volume_up'}</span>
                    </button>
                </div>
            </div>
        </div>
